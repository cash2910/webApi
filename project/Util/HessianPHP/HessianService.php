...
class HessianService{
	...
	public function handle(){
		...
		$payload = '';
		ob_start(); 
		file_put_contents('aaa.txt', "000\n", FILE_APPEND);
		try{
			file_put_contents('aaa.txt', "aaaa\n", FILE_APPEND);
			$instream->readAll();
			$parser = $this->factory->getParser($instream, $this->options);
			file_put_contents('aaa.txt', $parser."ddd\n", FILE_APPEND);
			$parser->setTypeMap($this->typemap);
			// cache version detection
			file_put_contents('aaa.txt', 'aa111',FILE_APPEND);
			$writer = $this->factory->getWriter($instream, $this->options);
			file_put_contents('aaa.txt', 'aa222',FILE_APPEND);
			$writer->setTypeMap($this->typemap);
			file_put_contents('aaa.txt', '222',FILE_APPEND);
			...
			file_put_contents('aaa.txt', "bbbb\n", FILE_APPEND);
		}
