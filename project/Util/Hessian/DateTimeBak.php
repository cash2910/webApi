...
class DateTimeBak{
	...
	function isBefore($time){
		if(DateTime::compare($this,$time) == -1)
			return true;
		return false;
	}
	...
	function equals($time){
		if(DateTime::compare($this,$time) == 0)
			return true;
		return false;
	}
	...
	function compare($time1,$time2){
		// delegamos encontrar el tipo de lo que se este pasando a otra función, 
		// una especie de overload
		$ts1 = DateTime::findTimestamp($time1);
		$ts2 = DateTime::findTimestamp($time2);
	...
	function findTimestamp($dateObj){
		if(is_object($dateObj) && is_a($dateObj,'DateTime'))
	...
	function __dateOperation($op,$part,$num){
		...
		if(!in_array($part,$parts)) {
			return trigger_error("operation: '$part' is not a part of DateTime",E_USER_WARNING);
		}
		switch($op){
			case 'add':
